...
Авторские уведомления, кнопки и интеграции можно добавить через необязательный
`user.js`. Скопируйте `user-example.js` в `user.js`: учебный пример содержит
уведомление, встроенные стили и вымышленную аналитику без отправки данных.
Перед адаптацией прочитайте [руководство по user.js](docs/user-script.md).
...
- `vn-vertical-engine-latest-update.zip` — обновление движка без папки `assets/`,
  пользовательских `story.js`, `user.js` и корневых `story-example.js`,
  `user-example.js`. Актуальные примеры находятся в `docs/examples/`.
...
├── user-example.js                    учебный пример авторского интерфейса
├── user.js                            необязательное расширение автора
...
В релизном update-архиве отсутствуют `assets/`, `story.js`, `user.js` и корневые
`story-example.js` и `user-example.js`, поэтому обновление не заменяет материалы и сценарий
существующей новеллы. Актуальный пример остаётся в
`docs/examples/story-example.js`, пример расширения — в `docs/examples/user-example.js`.
